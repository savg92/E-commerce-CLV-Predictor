...
export function App({ onPredict }: AppProps) {
  const [formState, setFormState] = useState<PredictRequest>(DEFAULT_FORM_STATE);
  ...
  const predict = useMemo(
    () =>
      onPredict ??
      createPredictClient(import.meta.env.VITE_API_BASE_URL ?? 'http://localhost:8000'),
    [onPredict],
  );
    ...
    <main className="dashboard-shell">
      <section className="hero-card">
        ...
        <p className="hero-copy">
          Enter a customer&apos;s RFM profile, preview batch CSV uploads, and surface prediction
          uncertainty clearly when the input drifts outside the training distribution.
        </p>
      </section>
      ...
      <div className="dashboard-grid">
        <form className="card panel" onSubmit={handleSubmit}>
          <div className="section-heading">
            ...
            <p>Provide the current RFM feature set for a single customer profile.</p>
          </div>
        ...
        <section className="card panel" aria-label="Batch CSV loader">
          <div className="section-heading">
            <h2>Batch CSV loader</h2>
            <p>Drop in a CSV file to preview the number of customers queued for batch prediction.</p>
          </div>
      ...
      <section className="card panel result-panel" aria-label="Prediction results">
        <div className="section-heading">
        ...
        {result ? (
          <>
            <div className="result-grid">
              <div>
                ...
                <strong className="result-value">${result.prediction.toFixed(2)}</strong>
              </div>
              <div>
                <span className="result-label">OOD score</span>
                <strong className="result-value">{result.anomaly_score.toFixed(3)}</strong>
              </div>
            </div>
          ...
          <p className="muted">Run a prediction to see the model output and uncertainty banner.</p>
        )}
      </section>
    </main>
  );
