...
8. **Record Task Progress:**
   ...
   - **Step 8.2: Update Plan:** Read `plan.md`, find the line for the completed task, update its status from `[~]` to `[x]`, and append the task summary directly in the plan for later consolidation into the phase commit message.

9. **Stage Changes:** Stage all code changes related to the task. **Do not commit yet** (commits are performed per phase).
...
[optional body containing task summaries]
...
7. Changes staged for phase commit
